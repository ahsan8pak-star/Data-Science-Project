...
class WAVGUIPlayer:
    """(AI) Main GUI window wrapping the TUI WAVAudioPlayer in a tkinter interface."""

    def __init__(self, root):
        """(AI) Initialises the main window and instance state."""
        self.root = root
        self.root.title("WAV GUI Player")
        self.root.geometry("500x450")
        self.root.resizable(False, False)

        self.player = None          # (AI) Holds the WAVAudioPlayer instance once a folder is selected
        self.wav_files = []         # (AI) List of .wav filenames loaded from the chosen directory
    ...
    def _build_ui(self):
        ...
        tk.Button(folder_frame, text="Select WAV Folder", command=self._select_folder).pack()  # Button that triggers _select_folder()

        # Track listbox area: a scrollable list showing all discovered .wav files
        list_frame = tk.Frame(self.root)
        list_frame.pack(fill=tk.BOTH, expand=True, padx=10)

        self.track_listbox = tk.Listbox(list_frame, height=10)  # Scrollable listbox showing all discovered .wav files
    ...
    def _select_folder(self):
        ...
        folder = filedialog.askdirectory(title="Select WAV Folder")  # Opens native directory picker dialog
        if not folder:
            return

        self.player = WAVAudioPlayer(folder)          # import tui/wav_tui_player.py: creates a new player instance for the chosen folder
        self.wav_files = self.player.get_wav_files()  # import tui/wav_tui_player.py: returns a list of .wav filenames in the folder
        ...
        if not self.wav_files:                        # Shows an error popup if no .wav files are found
            messagebox.showerror("Error", f"No .wav files found in:\n{folder}")
            return

        for idx, song in enumerate(self.wav_files, start=1):  # Populates the listbox with numbered track names
            self.track_listbox.insert(tk.END, f"[{idx}] {song}")

        self.status_label.config(text=f"Tracks found: {len(self.wav_files)}")  # Updates the status label with track count
    ...
    def _play(self):
        ...
        if not self.player or not self.wav_files:
            return

        self.player.play(self.wav_files[self.current_idx], track_num=self.current_idx + 1)  # import tui/wav_tui_player.py: loads and plays the .wav file via pygame.mixer
    ...
    def _pause(self):
        ...
        if self.player:
            self.player.pause()  # import tui/wav_tui_player.py: pauses playback via pygame.mixer.music.pause()
            self._update_status()
    ...
    def _resume(self):
        ...
        if self.player:
            self.player.resume()  # import tui/wav_tui_player.py: unpauses playback via pygame.mixer.music.unpause()
            self._update_status()
    ...
    def _forward(self):
        ...
        self.current_idx = self.player.forward(self.wav_files, self.current_idx)  # import tui/wav_tui_player.py: increments index (+1) or picks random if shuffle
    ...
    def _backward(self):
        ...
        self.current_idx = self.player.backward(self.wav_files, self.current_idx)  # import tui/wav_tui_player.py: decrements index (-1) or picks random if shuffle
        self.track_listbox.selection_clear(0, tk.END)   # Clears the current listbox highlight
        self.track_listbox.selection_set(self.current_idx)  # Highlights the newly selected track
        self._update_status()
    ...
    def _stop(self):
        ...
        if self.player:
            self.player.stop_music_file()  # import tui/wav_tui_player.py: stops playback via pygame.mixer.music.stop()
            self._update_status()
