...
class MP3AudioPlayer:
    ...
    def get_mp3_files(self):
        """Retrieves a list of .mp3 files from the target directory."""
    ...
    def play(self, song_name, track_num=None):
        ...
        try:
            ...
            display_info_box(["[P] Playing Music MP3 Player"], prepend_newline=True)
            self.show_status()
            return True
    ...
    def forward(self, mp3_files, current_idx):
        ...
        if self.shuffle and len(mp3_files) > 1:
            available_indices = [i for i in range(len(mp3_files)) if i != current_idx]
            next_idx = random.choice(available_indices)
        else:
            if current_idx + 1 >= len(mp3_files) and not self.loop_playlist:
                display_info_box(["[!] End of playlist reached (Loop Playlist is OFF)."], prepend_newline=True)
                return current_idx
            next_idx = (current_idx + 1) % len(mp3_files)

        display_info_box([f"[>>] Forward (-> Track {next_idx + 1})"], prepend_newline=True)
        self.play(mp3_files[next_idx], track_num=next_idx + 1)
        return next_idx

    def backward(self, mp3_files, current_idx):
        ...
        if self.shuffle and len(mp3_files) > 1:
            ...
            if current_idx - 1 < 0 and not self.loop_playlist:
                ...
                return current_idx
            prev_idx = (current_idx - 1) % len(mp3_files)

        display_info_box([f"[<<] Backward (-> Track {prev_idx + 1})"], prepend_newline=True)
        self.play(mp3_files[prev_idx], track_num=prev_idx + 1)
        return prev_idx
...
def mp3_player():
    music_folder = r"C:\Users\A.I.M\C.S\MP3"
    player = MP3AudioPlayer(music_folder)

    display_tui_banner("MP3 PLAYER")

    mp3_files = player.get_mp3_files()

    if not mp3_files:
        display_info_box([f"[X] Error: No .mp3 files found in folder:\n{music_folder}"], prepend_newline=True)
        return

    display_info_box([
        f"Directory    : {music_folder}",
        f"Tracks Found : {len(mp3_files)}"
    ])

    while True:
        print("\nAvailable Song List:")
        for idx, song in enumerate(mp3_files, start=1):
            print(f"[{idx}] {song}")
        ...
        if choice_input.upper() == "Q":
            ...
            display_info_box(["Shutting down MP3 Player interface..."], prepend_newline=True)
            break
        ...
        if not (0 <= choice_idx < len(mp3_files)):
            display_info_box(["[X] Error: Selected track index out of range."], prepend_newline=True)
            continue

        player.play(mp3_files[choice_idx], track_num=choice_idx + 1)
        ...
        while in_track_menu:
            ...
            match cmd:
                ...
                case "4":
                    choice_idx = player.forward(mp3_files, choice_idx)

                case "5":
                    choice_idx = player.backward(mp3_files, choice_idx)
...
if __name__ == "__main__":
    try:
        ...
        print(f"\n\n{'=' * TOTAL_WIDTH}")
        print("[!] MP3 player session terminated by user.".center(TOTAL_WIDTH))
