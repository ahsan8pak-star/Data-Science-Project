...
class TestLists:
    ...
    def test_indexing_and_slicing_prints_before_the_crash(self):
        ...
        # reaches the end, so we only get to assert on what it printed
        # up to that point.
    ...
    def test_pop_from_cleared_list_crashes(self):
        ...
        # immediately followed by `numbers.pop()` on the very next line,
        # so the script always raises IndexError at that point and never
